...
from backend.app.matrix.models import MatrixPuzzle
...
def test_matrix_generator_returns_contract_validated_puzzle_for_single_rule() -> None:
    puzzle = MatrixGenerator(RotationRule()).generate(seed=101)

    assert isinstance(puzzle, MatrixPuzzle)
    puzzle.validate_contract()
    assert puzzle.missing_position == (2, 2)
    assert puzzle.quality_score is not None
