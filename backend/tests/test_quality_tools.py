...
def test_statistical_validation_report_shape() -> None:
    ...
    assert isinstance(report["candidate_puzzles_evaluated"], int)
    assert isinstance(report["rejection_rate"], float)
    assert isinstance(report["average_generation_attempts"], float)
    assert isinstance(report["ambiguity_rate"], float)
    assert isinstance(report["duplicate_distractor_rate"], float)
    assert isinstance(report["most_common_rejection_reasons"], list)
